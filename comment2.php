...
<body>
	<div class="main">
		...
		$sqlCount = "SELECT count(*) FROM br1";
		...
		$sql = "SELECT * FROM br1 where id = " . $currentPage . " order by id";
		$result = mysqli_query($conn, $sql);
		$sql01 = "SELECT * FROM br2 where id = " . $currentPage . " order by id";
		...
		<div class="tablebox">
			<table class="table table-hover">
				<tr>
					<th width=5% class="table-secondary idth">번호</th>
					<th width=80% class="table-secondary commentth">코멘트</th>
				...
				for (; $row = mysqli_fetch_array($result);) {
					...
					for ($countA = 1; $countA < 11; $countA++) {
						echo "
				<tr>
				<form action = '/action_php/comment_update2.php' method='POST' target = '_blank'>
					<td>" . $countA . "</td>
					...
					<input class='btn btn-primary btn-secondary' type='submit' value='수정'></td>
					</form>
				</tr>
			";
					}
		...
		<form action='/action_php/comment_call3.php' method='POST' target='_blank'>
			<div class="statusbox">
				<p class="btn alert-primary text-dark">설정2</p>
				<input type='hidden' name='reload' value="3">
				<input class='btn btn-primary btn-secondary' type='submit' value='새로고침' style="margin-top: -17px;">
			</div>
		</form>
			...
			<p>
				...
				for ($countPAGE = 1; $countPAGE <= $rowCount["count(*)"]; $countPAGE++) {
					...
					if ($countPAGE == $currentPage) {
						echo "
						<a class='pagenum' style='color:rgb(235, 57, 57); font-size:25px;' href='/comment2.php?currentPage=" . $countPAGE . "'>" . $countPAGE . "</a>
						";
					} else {
						echo "
						<a class='pagenum' href='/comment2.php?currentPage=" . $countPAGE . "'>" . $countPAGE . "</a>
						";
					}
				}
	...
	<?php
		...
		if ($currentPage > 1) {
			...
			echo "<a id ='before' class='btn btn-secondary before' href ='/comment2.php?currentPage=" . ($currentPage - 1) . "'> ◀ </a>&nbsp;&nbsp;&nbsp;&nbsp;";
		}
		...
		if ($currentPage < $lastPage) {
			...
			echo "<a id ='after' class='btn btn-secondary after' href='/comment2.php?currentPage=" . ($currentPage + 1) . "'> ▶ </a>";
		}
		mysqli_close($conn);
		?>
	</div>
