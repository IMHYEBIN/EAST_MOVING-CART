...
<body>
	<div class="main">
		...
		$sqlCount = "SELECT count(*) FROM cr1";
		...
		$sql = "SELECT * FROM cr1 where id = " . $currentPage . " order by id";
		$result = mysqli_query($conn, $sql);
		$sql01 = "SELECT * FROM cr2 where id = " . $currentPage . " order by id";
			...
			<table class="table table-hover">
				...
				for (; $row = mysqli_fetch_array($result);) {
					...
					for ($countA = 1; $countA < 11; $countA++) {
						echo "
				<tr>
				<form action = '/action_php/comment_update3.php' method='POST' target = '_blank'>
					<td>" . $countA . "</td>
		...
		<form action='/action_php/comment_call3.php' method='POST' target='_blank'>
			<div class="statusbox">
				<p class="btn alert-primary text-dark">설정3</p>
			...
			<p>
				...
				for ($countPAGE = 1; $countPAGE <= $rowCount["count(*)"]; $countPAGE++) {
					...
					if ($countPAGE == $currentPage) {
						echo "
						<a class='pagenum' style='color:rgb(235, 57, 57); font-size:25px;' href='/comment3.php?currentPage=" . $countPAGE . "'>" . $countPAGE . "</a>
						";
					} else {
						echo "
						<a class='pagenum' href='/comment3.php?currentPage=" . $countPAGE . "'>" . $countPAGE . "</a>
						";
					}
				}
		...
		if ($currentPage > 1) {
			...
			echo "<a id ='before' class='btn btn-secondary before' href ='/comment3.php?currentPage=" . ($currentPage - 1) . "'> ◀ </a>&nbsp;&nbsp;&nbsp;&nbsp;";
		}
		...
		if ($currentPage < $lastPage) {
			...
			echo "<a id ='after' class='btn btn-secondary after' href='/comment3.php?currentPage=" . ($currentPage + 1) . "'> ▶ </a>";
		}
		mysqli_close($conn);
		?>
		&nbsp;&nbsp;
