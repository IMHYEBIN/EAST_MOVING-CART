...
<body>
	...
	$sql1 = "select * from main_viewer where item_id_num = 0 order by id desc limit 3"; 		// 문구 보기
	...
	$sql00 = "select selected_item from check1";
	$res00 = mysqli_query($conn, $sql00);
	$row00 = mysqli_fetch_array($res00);

	if ($row00['selected_item'] == '==제품전체==') {
		$temp0 = "";
	} else if ($row00['selected_item'] != 'null') {
		$temp0 = "where lot_num = '" . $row00['selected_item'] . "'";
	} else {
		$temp0 = "";
	}

	$sql3 = "select * from result1 " . $temp0 . " order by id desc";
	$res3 = mysqli_query($conn, $sql3);
	$row3 = mysqli_fetch_array($res3);

	$cycleTime = $row3[22];  	    //cycleTime

	$result001 = $row3[16];					//미성형 검사 1항목
	$result002 = $row3[17];					//미성형 검사 2항목
	$result003 = $row3[11];			//치수검사 1항목
	$result004 = $row3[12];			//치수검사 2항목
	$result005 = $row3[13];			//치수검사 3항목
	$result006 = $row3[14];			//치수검사 4항목
	$result007 = $row3[15];			//치수검사 5항목
	$result008 = 0;			//치수검사 6항목
	$result009 = 0;			//치수검사 7항목
	$result010 = 0;			//치수검사 8항목
	//검사 결과값
	...
	<section>
		<div class="status_container">
			<form method="POST" action="/item_update.php" target="_self">
				<h3>
					<div class="alert-primary status rounded"><span class="text-dark">생산 수량: <?php echo number_format($sumcount, 0); ?></span></div>
					<div class="alert-success status rounded"><span class="text-dark">양품 수량: <?php echo number_format($okcount, 0); ?></span></div>
					<div class="alert-danger status rounded"><span class="text-dark">불량수량: <?php echo number_format($ngcount, 0); ?></span></div>
					<div class="alert-warning status rounded"><span class="text-dark">Cycle Time: <?php echo number_format($cycleTime, 0); ?></span></div>
					<div class="status">
						<!-- <select id="item_id_num" onchange="location.href=this.value">
						<option value="/action_php/main/main_viewer0(search).php">제품선택</option>
						...
						<select name='selected_item'>
							...
							for (; $row5 = mysqli_fetch_array($res5);) {
								if($row00['selected_item'] == $row5['item_name']){echo "<option value='" . $row5['item_name'] . "' selected>" . $row5['item_name'] . "</option>";}
								else{echo "<option value='" . $row5['item_name'] . "'>" . $row5['item_name'] . "</option>";}
							}

							// $sql6 = "select * from result1 where lot_num = '" . $row5['item_name'] . "' order by id desc limit 1";
					...
					<div class="status">
						<button class="btn btn-secondary" type="submit">제품변경</button>
					</div>
					<div class="status">
						<a class="btn btn-secondary" name="item_add" onclick="popup_add()">제품등록</a>
					</div>
				</h3>
			</form>
		...
		<div style="float:right; padding-right:610px;">
			<!-- <button type="button" onclick="location.href='kimbeumsu://'">스크린샷</button> -->
		</div>
		<!-- 1번째 분활 화면 시작 -->
		<section>
			<table width="100%">
				<thead>
					...
					<tr>
						<td colspan="2" bgcolor="#EAEAEA"><span class="strong">LVDT 01</span></td>
						<td colspan="2" bgcolor="#EAEAEA">
							<?php echo $data1_min; ?> mm ~ <?php echo $data1_max; ?> mm</td>
						<!--검사 기준치 minSpec1 ~ maxSpec1 단위= mm -->
						<td colspan="2"><?php echo $result001; ?></td>
					</tr>
					<tr>
						<td colspan="2" bgcolor="#EAEAEA"><span class="strong">LVDT 02</span></td>
						<td colspan="2" bgcolor="#EAEAEA">
							<?php echo $data2_min; ?> mm ~ <?php echo $data2_max; ?> mm</td>
						<!--검사 기준치 minSpec2 ~ maxSpec3 단위= mm -->
						<td colspan="2"><?php echo $result002; ?></td>
					</tr>
					<tr>
						<td colspan="2" bgcolor="#EFF7FF"><span class="strong">동심 A</span></td>
						<td colspan="2" bgcolor="#EFF7FF">
							<?php echo $data3_min; ?> mm ~ <?php echo $data3_max; ?> mm</td>
						<!--검사 기준치 minSpec3 ~ maxSpec3 단위= mm -->
						<td colspan="2"><?php echo $result003; ?></td>
					</tr>
					<tr>
						<td colspan="2" bgcolor="#EFF7FF"><span class="strong">동심 B</span></td>
						<td colspan="2" bgcolor="#EFF7FF">
					...
					<tr>
						<td colspan="2" bgcolor="#EFF7FF"><span class="strong">동심 C</span></td>
						<td colspan="2" bgcolor="#EFF7FF">
					...
					<tr>
						<td colspan="2" bgcolor="#EFF7FF"><span class="strong">동심 D</span></td>
						<td colspan="2" bgcolor="#EFF7FF">
					...
					<tr>
						<td colspan="2" bgcolor="#EAEAEA"><span class="strong">조도</span></td>
						<td colspan="2" bgcolor="#EAEAEA">
					...
					<tr>
						<td colspan="2" bgcolor="#EAEAEA"><span class="strong"></span></td>
  ...
  <article> 
	...
	<script>
		function popup_add() {
			var url = "/item_add.php";
			var name = "item_add";
			var option = "width = 600, height = 260, top = 100, left = 200, location = no"
