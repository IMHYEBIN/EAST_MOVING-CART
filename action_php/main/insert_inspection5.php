...
$sql1 = "select * from main_viewer where item_id_num = 5 order by id desc limit 3"; 		// 문구 보기
...
<body>
    ...
    <input type="hidden" name="item_id_num" value="5">
...
<table>
    ...
    <tr>
        <td colspan="2" bgcolor="#FFFFA1"><span class="strong">외관검사 1</span></td>
        <td colspan="4"><?php if($shape1_check==0) {echo "무검사";} else {echo "유검사";} ?>
									<select name="shape1_check" id="s_check">
									<optgroup label="외관 검사 진행 :">
									<option value="0" selected>--</option>
									<option value="1">유</option></td>
    </tr>
    <tr>
        <td colspan="2" bgcolor="#FFFFA1"><span class="strong">외관검사 2</span></td>
        <td colspan="4"><?php if($shape2_check==0) {echo "무검사";} else {echo "유검사";} ?>
									<select name="shape2_check" id="s_check">
									<optgroup label="외관 검사 진행 :">
									<option value="0" selected>--</option>	
									<option value="1">유</option></td>
    <tr>
        <td colspan="2" bgcolor="#FFFFA1"><span class="strong">외관검사 3</span></td>
        <td colspan="4"><?php if($shape3_check==0) {echo "무검사";} else {echo "유검사";} ?>
									<select name="shape3_check" id="s_check">
									<optgroup label="외관 검사 진행 :">
									<option value="0" selected>--</option>	
									<option value="1">유</option></td>
    </tr>
    <tr>
        <td colspan="2" bgcolor="#FFFFA1"><span class="strong">DIM①</span></td>
	...
	<tr>
		...
		<td colspan="4"><input type="text" size="3" name="data6_min" value="<?php echo $data6_min; ?>" oninvalid="this.setCustomValidity('해당 치수가 없을 시 0이라도 입력 하여주세요!')" oninput="setCustomValidity('')" required> mm ~ 
                        <input type="text" size="3" name="data6_max" value="<?php echo $data6_max; ?>" oninvalid="this.setCustomValidity('해당 치수가 없을 시 0이라도 입력 하여주세요!')" oninput="setCustomValidity('')" required> mm</td>		<!--검사 기준치 minSpec5 ~ maxSpec5 단위= mm -->
	</tr>
	<tr>
		<td colspan="2" bgcolor="#FFFFA1"><span class="strong">DIM⑦</span></td>
		<td colspan="4"><input type="text" size="3" name="data7_min" value="<?php echo $data7_min; ?>" oninvalid="this.setCustomValidity('해당 치수가 없을 시 0이라도 입력 하여주세요!')" oninput="setCustomValidity('')" required> mm ~ 
                        <input type="text" size="3" name="data7_max" value="<?php echo $data7_max; ?>" oninvalid="this.setCustomValidity('해당 치수가 없을 시 0이라도 입력 하여주세요!')" oninput="setCustomValidity('')" required> mm</td>		<!--검사 기준치 minSpec5 ~ maxSpec5 단위= mm -->
	</tr>
    <tr>
        <td colspan="6" style="height:5px"></td>
    </tr>
