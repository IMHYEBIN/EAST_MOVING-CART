...
$sql = "select * from result1 where date = '".$date_call."' and time >= '".$times1."' and time <= '".$time_call."' order by id desc";
$res = mysqli_query($conn, $sql);                                     //검사 수량

$sql1 = "select * from result1 where date = '".$date_call."' and time >= '".$times1."' and time <= '".$time_call."' order by id desc";
$res1 = mysqli_query($conn, $sql1);                                   //y data 값 산출

$sql2 = "select * from result1 where date = '".$date_call."' and time >= '".$times1."' and time <= '".$time_call."' order by id desc";
$res2 = mysqli_query($conn, $sql2);                                   //y에 찍히는 data값 sum

$sql3 = "select * from result1 where date = '".$date_call."' and time >= '".$times1."' and time <= '".$time_call."' order by id desc";
...
<table>
  <tr align="center">
  <th bgcolor="#cce5ff" width="80px" height="10px">품 목</th>
    <td width="100px">
<p><?php
  if ($item_no == 0) {
    $select_item = "아이템1";
...
echo $select_item; ?></p>
  ...
  <th bgcolor="#cce5ff" width="80px" height="10px">DIM ①</th>
  <td><?php
  echo $data1_min.'mm ~ '.$data1_max.' mm';
  ?></td>
</tr>
</table>
<!-- 그래프 시작 -->
...
// horizontalLine block
  const horizontalLine = {
    ...
    beforeDraw(chart, args, options) {
      ...
      ctx.fillRect(left, y.getPixelForValue(<?php echo $data1_max; ?>), width, 2);
      // 2 draw a line //센터치
      ctx.fillStyle = options.lineColor_main;                                      //CENTER / *여기서 직접 지정도 가능
      ctx.fillRect(left, y.getPixelForValue(<?php echo $data1; ?>), width, 0.8);
      // 3 draw a line //하한치
      ctx.fillStyle = options.lineColor_main;                                      //LSL / *보조라인 색상에 따라 변경가능
      ctx.fillRect(left, y.getPixelForValue(<?php echo $data1_min; ?>), width, 2);
  ...
  const config = {
    ...
    data,
      options: {
          scales: {
            y: {
              max : <?php $max1=$data1_max+0.05; echo $max1; ?>,   //y 축 반응형으로 최대값 변경 가능 / 현재는 설정치(공차 최대값)에서 +0.5
              min : <?php $min1=$data1_min-0.05; echo $min1; ?>,   //y 축 반응형으로 최소값 변경 가능 / 현재는 설정치(공차 최소값)에서 -0.5
