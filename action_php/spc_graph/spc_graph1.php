...
<!-- 항목 변경시 수정 요망 -->
  ...
  <td><?php
  // 항목 변경시 수정 요망
  echo $data2_min.'mm ~ '.$data2_max.' mm';
  ?></td>
</tr>
</table>
<!-- 그래프 시작 -->
...
<script>
  ...
  const data = {
        ...
        datasets: [{
            label: '# spc data',
            data: [<?php for(;$row1 = mysqli_fetch_array($res1);) {$result1 = $row1[10]; echo "'".number_format($result1,2)."',";} ?>],  // 아이템 or 항목 변경시 수정 요망
            fill: false,
...
<?php 
// avr = sum/n  <== *평균
for($countN=0;$row2 = mysqli_fetch_array($res2);$countN++){$sum += $row2[10];} $avr = $sum / $countN;

// v(분산) = pow(x-avr),2
for($t=0; $row3 = mysqli_fetch_array($res3);$t += pow(($row3[10]-$avr),2)){}
...
// 1 draw a line //상한치      //USL / *보조라인 색상에 따라 변경가능
      ...
      ctx.fillRect(left, y.getPixelForValue(<?php echo $data2_max; ?>), width, 2);
// 2 draw a line //센터치     //CENTER / *여기서 직접 지정도 가능
      ctx.fillStyle = options.lineColor_main;
      ctx.fillRect(left, y.getPixelForValue(<?php echo $data2; ?>), width, 0.8);
// 3 draw a line //하한치     //LSL / *보조라인 색상에 따라 변경가능
      ctx.fillStyle = options.lineColor_main;
      ctx.fillRect(left, y.getPixelForValue(<?php echo $data2_min; ?>), width, 2);
  ...
  const config = {
    ...
    data,
      options: {
          scales: {
            y: {
              max : <?php $max2=$data2_max+0.1; echo $max2; ?>,   //y 축 반응형으로 최대값 변경 가능 / 현재는 설정치(공차 최대값)에서 +0.5 #항목 변경시 $data1_ <== 변경해줘야함
              min : <?php $min2=$data2_min-0.1; echo $min2; ?>,   //y 축 반응형으로 최소값 변경 가능 / 현재는 설정치(공차 최소값)에서 -0.5 #항목 변경시 $data1_ <== 변경해줘야함
