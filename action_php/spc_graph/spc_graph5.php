...
echo $select_item; ?></p>
  ...
  <td><?php
  echo $data6_min.'mm ~ '.$data6_max.' mm';
  ?></td>
</tr>
</table>
<!-- 그래프 시작 -->
...
// horizontalLine block
  const horizontalLine = {
    ...
    beforeDraw(chart, args, options) {
      ...
      ctx.fillRect(left, y.getPixelForValue(<?php echo $data6_max; ?>), width, 2);
      // 2 draw a line //센터치
      ctx.fillStyle = options.lineColor_main;                                      //CENTER / *여기서 직접 지정도 가능
      ctx.fillRect(left, y.getPixelForValue(<?php echo $data6; ?>), width, 0.8);
      // 3 draw a line //하한치
      ctx.fillStyle = options.lineColor_main;                                      //LSL / *보조라인 색상에 따라 변경가능
      ctx.fillRect(left, y.getPixelForValue(<?php echo $data6_min; ?>), width, 2);
  ...
  const config = {
    ...
    data,
      options: {
          scales: {
            y: {
              max : <?php $max6=$data6_max+0.1; echo $max6; ?>,   //y 축 반응형으로 최대값 변경 가능 / 현재는 설정치(공차 최대값)에서 +0.1
              min : <?php $min6=$data6_min-0.1; echo $min6; ?>,   //y 축 반응형으로 최소값 변경 가능 / 현재는 설정치(공차 최소값)에서 -0.1
