...
$times1 = date("H:i", strtotime($time_call."-30 minutes"));  // 시 hours 분 minutes 외 다른건 관리 안함 기준값이 모호해짐 feat.김범수
...
$data7_min = $row0[18];			$data7_max = $row0[19];           $data7 = ($data7_min+$data7_max)/2;
  ...
  const config = {
    ...
    data,
      options: {
          scales: {
            y: {
              max : <?php $max1=$data1_max+0.1; echo $max1; ?>,   //y 축 반응형으로 최대값 변경 가능 / 현재는 설정치(공차 최대값)에서 +0.5
              min : <?php $min1=$data1_min-0.1; echo $min1; ?>,   //y 축 반응형으로 최소값 변경 가능 / 현재는 설정치(공차 최소값)에서 -0.5
